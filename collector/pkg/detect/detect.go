...
func (d *Detect) TransformDetectedDevices(detectedDeviceConns models.Scan) []models.Device {
	...
	for _, overrideDevice := range d.Config.GetDeviceOverrides() {
		...
		if overrideDevice.Ignore {
			// case-insensitive delete: the scanned entry may have been stored with different case
			for key := range groupedDevices {
				if strings.EqualFold(key, overrideDeviceFile) {
			...
			if overrideDevice.DeviceType != nil {
				...
				if scannedKey != "" {
					scannedDevice := groupedDevices[scannedKey]
			...
			for key := range groupedDevices {
				if strings.EqualFold(key, overrideDeviceFile) && key != overrideDeviceFile {
					delete(groupedDevices, key)
					break
				}
			}
