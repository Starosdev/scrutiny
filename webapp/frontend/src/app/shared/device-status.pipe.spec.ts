...
describe('DeviceStatusPipe', () => {
    ...
    describe('#deviceStatusForModelWithThreshold', () => {
        it('if healthy device, should be passing', () => {
            expect(DeviceStatusPipe.deviceStatusForModelWithThreshold({ device_status: 0 } as DeviceModel, true, MetricsStatusThreshold.Both)).toBe('passed');
        });

        it('if device with no smart data, should be unknown', () => {
            expect(DeviceStatusPipe.deviceStatusForModelWithThreshold({ device_status: 0 } as DeviceModel, false, MetricsStatusThreshold.Both)).toBe('unknown');
        });
