...
func NewReplacementRisk(logger logrus.FieldLogger, appconfig config.Interface, device *models.Device, score int, category models.RiskCategory) Notify {
	...
	rows := [][2]string{
		{"Failure Type", NotifyFailureTypeReplacementRisk},
		{"Risk Category", string(category)},
		{"Risk Score", fmt.Sprintf("%d/100", score)},
	...
	if hostId := strings.TrimSpace(device.HostId); len(hostId) > 0 {
		rows = append(rows, [2]string{"Host Id", hostId})
	}
	if label := strings.TrimSpace(device.Label); len(label) > 0 {
		rows = append(rows, [2]string{"Device Label", label})
	}
	rows = append(rows, [2]string{"Date", payload.Date})
	payload.HTMLMessage = formatNotificationHTML(
		payload.Subject,
		"Scrutiny replacement risk notification",
		"REPLACEMENT RISK",
		"#dc3545",
		rows,
