...
`)
	...
	if p.Test {
		b.WriteString(`<tr><td style="background-color:#0d6efd;padding:12px 30px;color:#ffffff;font-size:12px;font-weight:bold;letter-spacing:0.04em;">TEST NOTIFICATION</td></tr>`)
	}
...
<p style="margin:0;color:#6c757d;font-size:13px;">Scrutiny SMART error notification</p>
</td></tr>`, html.EscapeString(p.Subject))
...
<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:14px;color:#212529;">`)
	...
	writePayloadHTMLRow(&b, "Failure Type", p.FailureType)
	writePayloadHTMLRow(&b, "Device", deviceIdentifier)
	writePayloadHTMLRow(&b, "Device Name", p.DeviceName)
	writePayloadHTMLRow(&b, "Device Serial", p.DeviceSerial)
	writePayloadHTMLRow(&b, "Device Type", p.DeviceType)
	if len(p.HostId) > 0 {
		writePayloadHTMLRow(&b, "Host Id", p.HostId)
	}
	if len(p.DeviceLabel) > 0 {
		writePayloadHTMLRow(&b, "Device Label", p.DeviceLabel)
	}
	writePayloadHTMLRow(&b, "Date", p.Date)

	b.WriteString(`</table></td></tr>
<tr><td style="padding:15px 30px;background-color:#f8f9fa;border-top:1px solid #dee2e6;">
<p style="margin:0;color:#6c757d;font-size:11px;">Generated by Scrutiny</p>
</td></tr>
</table>
</td></tr></table>
</body></html>`)
