...
func (p *Payload) GenerateHTMLMessage() string {
	...
	bannerColor := ""
	if p.Test {
		bannerText = "TEST NOTIFICATION"
		bannerColor = "#0d6efd"
	}

	return formatNotificationHTML(
		p.Subject,
		"Scrutiny SMART error notification",
		bannerText,
		bannerColor,
		rows,
...
func payloadDeviceIdentifier(deviceName, deviceLabel string) string {
	if len(deviceLabel) == 0 {
		return deviceName
	}
	return fmt.Sprintf(fmtLabelWithName, deviceLabel, deviceName)
}
...
func (p *HeartbeatPayload) generateHTMLMessage() string {
	rows := [][2]string{
		{"Failure Type", p.FailureType},
		{"Monitored Devices", strconv.Itoa(p.MonitoredDevices)},
		{"Total Devices", strconv.Itoa(p.TotalDevices)},
		{"Date", p.Date},
	}
	return formatNotificationHTML(
		p.Subject,
		"Scrutiny heartbeat notification",
		"ALL CLEAR",
		"#28a745",
		rows,
		"This automated heartbeat confirms that Scrutiny is running and all monitored drives are healthy.",
	)
}
...
func NewHeartbeat(logger logrus.FieldLogger, appconfig config.Interface, monitoredCount, totalCount int) Notify {
	...
	payload := Payload{
		...
		HTMLMessage: heartbeatPayload.HTMLMessage,
	}
...
func NewPerformanceDegradation(logger logrus.FieldLogger, appconfig config.Interface, device *models.Device, metric string, baselineAvg, currentValue, deviationPct float64) Notify {
	...
	payload.HTMLMessage = formatNotificationHTML(
		payload.Subject,
		"Scrutiny performance degradation notification",
		"PERFORMANCE ALERT",
		"#e6a817",
		[][2]string{
			{"Failure Type", degradationPayload.FailureType},
			{"Device", payloadDeviceIdentifier(degradationPayload.DeviceName, degradationPayload.DeviceLabel)},
			{"Device Name", degradationPayload.DeviceName},
			{"Device Serial", degradationPayload.DeviceSerial},
			{"Device WWN", degradationPayload.DeviceWWN},
			{"Metric", degradationPayload.Metric},
			{"Baseline Average", fmt.Sprintf("%.2f", degradationPayload.BaselineAvg)},
			{"Current Value", fmt.Sprintf("%.2f", degradationPayload.CurrentValue)},
			{"Deviation", fmt.Sprintf("%.1f%%", degradationPayload.DeviationPct)},
			{"Date", degradationPayload.Date},
		},
		"Generated by Scrutiny",
	)
...
func NewCollectorError(logger logrus.FieldLogger, appconfig config.Interface, device *models.Device, errorType, errorMessage string) Notify {
	...
	payload.HTMLMessage = formatNotificationHTML(
		payload.Subject,
		"Scrutiny collector error notification",
		"COLLECTOR ERROR",
		"#dc3545",
		[][2]string{
			{"Failure Type", NotifyFailureTypeCollectorError},
			{"Device", payloadDeviceIdentifier(deviceIdentifier, strings.TrimSpace(device.Label))},
