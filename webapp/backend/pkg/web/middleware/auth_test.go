...
func TestAuthMiddleware_AuthEnabled_PrivateDocsRoute_RequiresToken(t *testing.T) {
	router := setupRouter(t, true, testAPIToken, testJWTSecret, "")

	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/docs/api", nil)
	router.ServeHTTP(w, req)

	require.Equal(t, http.StatusUnauthorized, w.Code)
}

func TestAuthMiddleware_AuthEnabled_PrivateDocsRoute_WithAPIToken(t *testing.T) {
	router := setupRouter(t, true, testAPIToken, testJWTSecret, "")
...
func TestAuthMiddleware_AuthEnabled_PublicDocsRoute(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	fakeConfig.EXPECT().GetBool("web.docs.public").Return(true).AnyTimes()
	...
	for _, route := range []string{"/docs/api", pathOpenAPIDoc} {
		w := httptest.NewRecorder()
...
func TestAuthMiddleware_AutoGeneratedJWTSecret_WrittenBackToConfig(t *testing.T) {
	// When jwt_secret is empty, the middleware generates a random one and writes it back.
	// A JWT signed with that written-back secret must be accepted.
	gin.SetMode(gin.TestMode)
	...
	// First call returns empty (triggering generation), subsequent calls return what was Set().
	var writtenSecret string
	fakeConfig.EXPECT().GetString("web.auth.jwt_secret").DoAndReturn(func(key string) string {
		return writtenSecret
	}).AnyTimes()
	fakeConfig.EXPECT().Set("web.auth.jwt_secret", gomock.Any()).DoAndReturn(func(key string, value interface{}) {
		writtenSecret = value.(string)
	}).Times(1)
	fakeConfig.EXPECT().GetInt("web.auth.jwt_expiry_hours").Return(24).AnyTimes()
	...
	// The middleware should have written a secret back
	require.NotEmpty(t, writtenSecret, "middleware should write generated JWT secret back to config")

	// Generate a JWT with the written-back secret (simulating what the login handler does)
	jwtToken, _, err := auth.GenerateJWT(writtenSecret, 24)
	require.NoError(t, err)

	// The middleware should accept this JWT
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", pathSummary, nil)
	req.Header.Set("Authorization", bearerPrefix+jwtToken)
	r.ServeHTTP(w, req)

	require.Equal(t, http.StatusOK, w.Code, "JWT signed with auto-generated secret should be accepted")
}
