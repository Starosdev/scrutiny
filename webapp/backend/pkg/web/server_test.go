...
func helperCreateAPIDocsFiles(t *testing.T, docsPath string) {
	...
	err = ioutil.WriteFile(swaggerPath, []byte(`<html><body>swagger<script>window.ui={url: "/api/docs/openapi.yaml"}</script></body></html>`), 0644)
	require.NoError(t, err)
...
func (suite *ServerTestSuite) TestRegisterDevicesRoute_LegacySmartSupportBool() {
	parentPath, _ := ioutil.TempDir("", "")
	defer os.RemoveAll(parentPath)
	helperCreateFrontendFiles(suite.T(), parentPath)
...
func (suite *ServerTestSuite) TestGetDevicesSummaryRoute_Nvme() {
	...
	//assert
	require.Equal(suite.T(), "a4c8e8ed-11a0-4c97-9bba-306440f1b944", deviceSummary.Data.Summary["76866f4d-73af-5e26-b9a5-6bf6451092a9"].Device.WWN)
	require.Equal(suite.T(), pkg.DeviceStatusPassed, deviceSummary.Data.Summary["76866f4d-73af-5e26-b9a5-6bf6451092a9"].Device.DeviceStatus)
}
