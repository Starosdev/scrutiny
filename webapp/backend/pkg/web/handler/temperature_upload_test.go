...
func TestUploadTemperatureDeliversOnce(t *testing.T) {
	for _, stalled := range []bool{false, true} {
		t.Run(fmt.Sprint("stalled=", stalled), func(t *testing.T) {
			...
			repo.EXPECT().SaveSmartAttributes(gomock.Any(), "drive", gomock.Any()).Return(measurements.Smart{Temp: 60, Date: now, Status: pkg.DeviceStatusPassed}, nil).Times(2)
...
func TestUploadSharesNotificationSettings(t *testing.T) {
	for _, scenario := range []string{"available", "missing", "load error", "no gate", "nil gate", "quiet hours", "rate limit"} {
		t.Run(scenario, func(t *testing.T) {
			...
			repo.EXPECT().SaveSmartAttributes(gomock.Any(), "drive", gomock.Any()).Return(measurements.Smart{Temp: 60, Status: device.DeviceStatus}, nil)
