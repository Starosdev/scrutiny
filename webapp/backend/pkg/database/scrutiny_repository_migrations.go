...
func (sr *scrutinyRepository) Migrate(ctx context.Context) error {
	if sr.gormClient.Name() == dialectPostgres {
		return sr.migratePostgres(ctx)
	}
	...
	m := gormigrate.New(sr.gormClient, gormMigrateOptions, sr.schemaMigrations(ctx))
...
// schemaMigrations is the full relational migration history, in order.
func (sr *scrutinyRepository) schemaMigrations(ctx context.Context) []*gormigrate.Migration {
...
// migrationLockKey identifies the PostgreSQL advisory lock that serializes migrations.
const migrationLockKey int64 = 0x5343525554494e59 // "SCRUTINY"

// migratePostgres migrates a PostgreSQL database. A new database gets the baseline schema and
// default settings, and every migration in the history is recorded as applied, since those
// migrations target SQLite. Later migrations run as usual and must work on both databases.
//
// The whole run holds a transaction-scoped advisory lock, so replicas that start together migrate
// one at a time. The lock ends with the transaction, which keeps it safe behind a transaction-pooling
// proxy such as pgbouncer, where a session-level lock could outlive the connection that took it.
//
// The SQLite-only global migrations (VACUUM) do not apply and are not run.
func (sr *scrutinyRepository) migratePostgres(ctx context.Context) error {
	sr.logger.Infoln("Database migration starting (PostgreSQL)")
	err := sr.gormClient.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Exec("SELECT pg_advisory_xact_lock(?)", migrationLockKey).Error; err != nil {
			return err
		}
		options := gormigrate.DefaultOptions
		options.UseTransaction = false
		m := gormigrate.New(tx, options, sr.schemaMigrations(ctx))
		m.InitSchema(func(tx *gorm.DB) error {
			sr.logger.Infoln("New PostgreSQL database: creating baseline schema and default settings")
			return initializeBaseline(ctx, tx, sr.logger)
		})
		return m.Migrate()
	})
	if err != nil {
		sr.logger.Errorf("Database migration failed.\nPlease open a github issue at https://github.com/Staros-Labs/scrutiny with this error.\n%v", err)
		return err
	}
	sr.logger.Infoln("Database migration completed successfully")
	return nil
