...
// ZFSPoolPresence describes whether the latest authoritative inventory saw a pool.
type ZFSPoolPresence string

const (
	ZFSPoolPresencePresent ZFSPoolPresence = "present"
	ZFSPoolPresenceMissing ZFSPoolPresence = "missing"
	ZFSPoolPresenceStale   ZFSPoolPresence = "stale"
	ZFSPoolPresenceUnknown ZFSPoolPresence = "unknown"
)
...
// zfs list and exclude parity; both are 0 for pools last reported by a collector
// that predates them.
type ZFSPool struct {
	...
	Presence             ZFSPoolPresence `json:"presence" gorm:"-"`
}

// ResolvePresence derives current presence from authoritative inventory timestamps.
func (p *ZFSPool) ResolvePresence(now time.Time, staleAfter time.Duration) ZFSPoolPresence {
	if p.LastInventoryAt.IsZero() || p.LastSeenAt.IsZero() {
		return ZFSPoolPresenceUnknown
	}
	if now.Sub(p.LastInventoryAt) > staleAfter {
		return ZFSPoolPresenceStale
	}
	if !p.LastSeenAt.Before(p.LastInventoryAt) {
		return ZFSPoolPresencePresent
	}
	return ZFSPoolPresenceMissing
}
