...
- `GET /api/device/{id}/selftest` returns ATA SMART self-test history already recorded during normal SMART uploads, plus additive `data.health` metadata for the newest result and retained failure history. Each entry preserves raw `lifetime_hours` and includes nullable `effective_lifetime_hours`; null means the absolute power-on age could not be resolved. Results follow newest observed controller log order, rather than raw lifetime values. The separate `POST /api/device/{id}/selftest` route remains reserved for future ingestion work.
